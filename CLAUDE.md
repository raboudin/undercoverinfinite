...
- `frontend/` — Nuxt 4 app with a ported design system (Tailwind theme tokens + Vue components), **a playable local pass-and-play game** (v1), and a networked online lobby (`/salle` — see below).
- `api/` — NestJS backend with Prisma ORM. Four domain modules: `entitlements` (thèmes, niveaux de difficulté — catalogue identique pour tous), `words` (le tirage des mots par LLM), `auth` (comptes, profil, JWT en cookies, OAuth Google/Facebook) et `rooms` (salons en ligne, Socket.IO) — see below.

Game state stays client-side **for local pass-and-play only** — the online mode is the opposite, the server is authoritative there (see `rooms` below). Either way, the frontend calls `api/` for **the available themes and difficulty levels** (`GET /entitlements`) and for **the words of each game** (`POST /words/draw`). There is no local word list and no daily batch: words are generated on demand. **Le jeu est entièrement gratuit** — il n'y a pas de crédit, pas de pack, pas de quota.

**Le compte est optionnel, en local comme en ligne** : `/connexion` existe des deux côtés (API + front), mais aucune route ni aucun écran de jeu n'est protégé — créer ou rejoindre une salle en ligne marche tout aussi bien sans compte. Le dossier d'agent reste la fondation du lobby en ligne (nom/avatar repris automatiquement pour un agent connecté), pas un péage devant la partie.
...
npm run start:dev         # start Nest in watch mode at http://localhost:8000 (PORT in .env, defaults to 8000 everywhere — dev and containers alike)
...
- `frontend/app/app.vue` — the shell: sticky header (logo mark + `AccountButton` + online lobby link + music mute toggle) wrapping `<NuxtPage />` in a `max-w-lg` mobile-first column. Screens don't repeat that chrome.
...
- Routes: `/` is the game (`app/pages/index.vue`), `/connexion` l'écran de compte (`app/pages/connexion.vue`), `/salle` le lobby en ligne (`app/pages/salle/index.vue` + `[code].vue`), `/design-system` is the component demo that used to live in `app.vue`.
...
All game state is client-side; the API supplies the available themes/difficulties (`useEntitlements`) and the words of each game (`useWords`). If the API is unreachable, word draw fails — there is no offline word fallback by design.
...
- `app/composables/useGame.ts` — the whole rules engine as a `createGame({ rng })` **factory**, not a module singleton, so every test starts clean and can inject a deterministic PRNG. Phases: `setup → reveal → describe → vote → elimination → (describe | victory)`. **Deux fins, et deux seulement** : plus aucun undercover en vie (les loyaux gagnent), ou des undercovers **strictement plus nombreux** que les loyaux (`aliveUndercovers > aliveCivils`). À égalité la partie continue — c'est ce qui laisse une dernière manche en tête-à-tête, et ce n'est pas un oubli. `configure()` plafonne malgré tout les undercovers à `Math.floor((n - 1) / 2)` : ce n'est plus une question de règle mais d'intérêt, une table qui démarre à égalité se joue sur une seule élimination.
  ...
  - `configure()` and `replaySameTeam()` take a `GameSetup` (`{ pair }`) as a parameter — the engine no longer picks words itself. The page must call `words.draw()` first and only `configure()` on success. Replaying with the same team is a new game and costs a new draw.
- `app/composables/useEntitlements.ts` — catalogue de thèmes et niveaux de difficulté. **Aucune logique de droits n'est rejouée côté client** : le serveur envoie `themes` (liste de `ThemeCard`) et `difficulties` (liste de `DifficultyCard`), ce composable les relaie. Le jeu étant gratuit, la réponse est identique pour tous — il n'y a pas de notion de droits par joueur.
- `app/composables/useWords.ts` — `POST /words/draw` par partie, avec `{ theme, spicy, difficulty }`. Un échec (503 si le LLM est hors ligne) bloque le lancement — il n'y a pas de quota ni de boutique à proposer.
...
- `app/components/game/` — écrans de partie (`SetupScreen`, `ThemeButton`, `ThemeCarousel`, `DifficultySlider`, `SpicyToggle`, `GameTable`, `ArtSlot`, `RevealScreen`, `DescribeScreen`, `VoteScreen`, `EliminationScreen`, `VictoryScreen`), auto-importés à plat comme les autres dossiers. Ils sont présentationnels : props en entrée, emits en sortie, aucun état de jeu à eux hors du strict nécessaire local (carte retournée, accusation en attente, décompte). Ils composent le design system, ils ne l'étendent pas.
...
- `SetupScreen` tient **deux étapes** : le menu (thème + spicy toggle + difficulté, rien d'autre) puis la table (noms, undercovers). Tout ce qui dépend des joueurs présents attend la table ; le menu ne pose que la question qu'on se pose avant de sortir le téléphone du sac.
...
- Les accroches des thèmes (`tagline`) viennent du catalogue serveur — le front ne connaît pas la liste des thèmes, il ne peut pas inventer leur copie.
...
- Big binaries (the mp3) live in `public/`, not `assets/`, so Vite serves them directly instead of bundling them. Les illustrations de thèmes sont dans `assets/` : c'est ce qui permet à `import.meta.glob` de savoir lesquelles existent (voir ci-dessus).
...
Écran `/connexion` (`app/pages/connexion.vue`) + `app/components/auth/` (`AuthForm`, `AccountButton`, `AgentDossier`, `LogoutConfirm`, `DeleteAccountDialog`, déclarés dans `components:` de `nuxt.config.ts` comme les autres dossiers). `/connexion` sert les deux états : le formulaire pour un visiteur, le dossier (photo, nom de code, sorties) pour un agent identifié. Ce qu'il ne faut pas défaire :

- **Le nom dans l'en-tête est un lien vers `/connexion`** (`AccountButton`) : c'est le *seul* chemin vers le dossier une fois connecté. Le retirer rendrait l'écran de profil injoignable.
- **La déconnexion passe par `LogoutConfirm`**, des deux côtés (en-tête et dossier). Le bouton de l'en-tête est à portée de pouce sur un téléphone qui circule autour de la table en pleine partie.
- **`DeleteAccountDialog` reste ouvert quand l'API refuse** : la fenêtre porte l'erreur, sinon une faute de frappe obligerait à tout ressaisir. Sur succès il n'y a plus de dossier, le composant disparaît de lui-même.
- **La réduction en vignette (`~/utils/avatarImage`) est faite par la page, pas par `AgentDossier`** : elle a besoin d'un canvas, que happy-dom n'implémente pas — la garder dans le composant le rendrait impossible à monter en test. Le composant remonte le `File` tel quel.
- **C'est le navigateur qui redimensionne, jamais l'API** : une photo de téléphone pèse plusieurs mégaoctets, l'envoyer entière pour n'en garder que 256 px coûterait le téléversement complet sur un réseau mobile, et obligerait le serveur à embarquer un décodeur d'images. Le format est choisi en testant le rendu de `toDataURL('image/webp')` plutôt qu'en tenant une liste de navigateurs.
...
Pages `app/pages/salle/index.vue` (créer/rejoindre) et `app/pages/salle/[code].vue` (la salle en direct), `app/composables/useRoomSocket.ts`, `app/components/online/` (`LobbyScreen`, `OnlineRevealScreen`, `OnlineVoteScreen`, déclarés dans `components:` de `nuxt.config.ts` comme les autres dossiers). Un lien « Jouer en ligne » vit dans l'en-tête (`app.vue`). Ce qu'il ne faut pas défaire :

- **`createRoomSocket()` s'instancie localement dans la page**, pas via un plugin `$…` comme `$auth`/`$entitlements` : une socket est scopée à une salle et doit se fermer en quittant la route (`onBeforeUnmount`), là où `$auth`/`$entitlements` couvrent toute la session. Même schéma d'injection que `createWords`/`createEntitlements` (`request` = `$auth.authFetch`).
- **REST pour créer/rejoindre, Socket.IO pour tout le reste** — le composable suit la même coupure que le backend (voir module `rooms`) : `createRoom()`/`joinRoom()` sont de simples appels `authFetch`, tout le reste (`start`, `castVote`, `ackReveal`…) n'est qu'un `emit` sur la socket déjà ouverte.
- **Le jeton de siège vit en `localStorage`** (`undercover:room:<code>`), pas en cookie : `connect(code, token?)` le relit automatiquement si aucun jeton explicite n'est fourni, ce qui permet de revenir sur `/salle/<code>` après un rafraîchissement sans repasser par le formulaire de jointure.
- **`state` est remplacé en bloc à chaque `room:state`**, jamais fusionné — même philosophie que côté serveur (`rooms.service.ts`), pour ne jamais laisser deux morceaux d'état issus de deux diffusions différentes cohabiter.
- **`RevealScreen`/`VoteScreen` locaux ne sont PAS réutilisés en ligne** : leur séquencement (une carte à la fois sur un même téléphone, un nom tapé pour toute la table) suppose un appareil partagé. `OnlineRevealScreen`/`OnlineVoteScreen` sont des composants dédiés — chacun ne voit que sa propre carte, chacun vote pour son propre compte, en simultané, avec un tableau de votes qui se remplit en direct (le vote est public, voir `rooms` côté API). `DescribeScreen`/`EliminationScreen`/`VictoryScreen`, eux, sont réutilisés tels quels : déjà présentationnels, sans dépendance directe à `useGame`.
- **`toLocalPlayer()` (dans `salle/[code].vue`) comble l'écart de type** entre `OnlinePlayer` (`role`/`word` nullables, rédigés par le serveur) et `Player` local (non nullables), pour pouvoir réutiliser ces trois composants sans les dupliquer. Ce n'est sûr que parce qu'ils n'affichent jamais `role`/`word` d'un agent vivant hors phase victoire — si l'un d'eux change ce comportement, cet adaptateur doit être revu en même temps.
...
- **`entitlements` module** (`api/src/entitlements/`) — catalogue de thèmes et niveaux de difficulté. Route unique : `GET /entitlements` (public, pas d'authentification requise). Retourne `{ themes: ThemeCard[], difficulties: DifficultyCard[] }` — identique pour tous. Points à ne pas défaire :
  - **`catalog.ts` est la source de vérité unique** des thèmes et niveaux de difficulté. `THEMES` et `DIFFICULTIES` (avec `DIFFICULTY_IDS` : `evident | facile | normal | difficile | farfelu`, niveau 1–5) y vivent — le front ne rejoue aucune de ces définitions.
  - **Le prompt d'un thème ne sort jamais de l'API** : le catalogue expose `{id, label, tagline}`, pas `THEMES[id].prompt`. La `tagline` est de la copie de vitrine — le front affiche un thème par écran et n'a aucun moyen d'inventer cette copie.
  - **`subject` (`user:<id>` ou `device:<uuid>`) est l'identifiant universel** du demandeur — utilisé par `words` pour dédupliquer les tirages (`content_draws`). Le cookie `device_id` (httpOnly, un an) ancre le sujet d'un visiteur anonyme à son appareil, pas à son IP.
- **`words` module** (`api/src/words/`) — le tirage des mots. `POST /words/draw { theme, spicy, difficulty }` : tire une paire inédite pour ce sujet, génère si le pool est vide. Points à ne pas défaire :
  - **Plus de cron ni de lot quotidien.** Les paires vivent dans un pool par `(theme, spicy, difficulty)` et ne sont générées que quand le demandeur n'a plus rien d'inédit — par lots de 8, pour amortir l'appel LLM sur plusieurs parties. `ScheduleModule` a disparu d'`AppModule`.
  - **`POST` et non `GET`** : l'appel peut déclencher une génération LLM. Il ne doit pas se faire rejouer par un cache ou un préchargement.
  ...
  - **`spicy: true`** isole un pool séparé du même thème avec un prompt distinct (osé mais borné : rien d'explicite, rien d'illégal, aucun mineur). `difficulty` (1–5) contrôle l'éloignement sémantique voulu de la paire — un paramètre de prompt, pas un filtre sur un champ enum.
...
- **`auth` module** (`api/src/auth/`) — comptes, profil et sessions. Login classique (`POST /auth/register|login|refresh|logout`, `GET /auth/me`) plus OAuth Google/Facebook (`GET /auth/{google,facebook}[/callback]`), les deux chemins terminant sur la même émission de tokens. `GET /auth/providers` (public) dit au front quels boutons sociaux ce serveur peut honorer. Le dossier se modifie via `PATCH /auth/me` (nom de code), `PUT|DELETE /auth/me/avatar` (photo) et `DELETE /auth/me` (effacement RGPD) ; `GET /auth/avatar/:userId` sert l'image. Points à ne pas défaire :
  - **`toPublicUser` + `PUBLIC_USER_INCLUDE` (`user.mapper.ts`) sont le seul chemin vers un `PublicUser`.** Six endroits lisent un utilisateur pour le client (register, login, les trois branches d'OAuth, `JwtStrategy`, la rotation de token) : sans jointure partagée, une session relue après rotation annoncerait une photo différente de celle lue par `/auth/me`.
  - **La photo vit dans sa propre table** (`user_avatars`), pas en colonne de `users` : cette ligne-là est relue à *chaque* requête authentifiée par `JwtStrategy` et n'a pas à traîner des kilo-octets d'image. En base et non sur un volume : l'API est un conteneur remplacé à chaque déploiement.
  - **`PublicUser` ne porte pas l'image mais `avatarUpdatedAt`**, dont le front fait un `?v=` sur l'URL de la photo. C'est ce qui permet à `GET /auth/avatar/:userId` d'être franchement caché sans jamais servir une photo périmée. Renvoyer la vignette en base64 dans `/auth/me` alourdirait chaque lecture de session.
  - **`parseAvatarDataUrl` (`avatar.ts`) vérifie les octets d'en-tête, pas seulement le type déclaré** : celui-ci vient du client, et ces octets sont resservis tels quels au navigateur. Pas de SVG (c'est un document, il porte du script), et `X-Content-Type-Options: nosniff` à la sortie.
  - **`MAX_AVATAR_BYTES` vaut 64 Kio parce qu'Express plafonne le corps JSON à 100 Ko.** La photo arrive en base64 : la relever sans relever aussi la limite du body parser transformerait un dépassement en 413 opaque, avant toute validation.
  - **Le mot de passe est réexigé pour supprimer un compte, et le refus sort en 403 — pas en 401.** La session est valable, c'est la re-authentification qui échoue ; un 401 déclencherait côté front la rotation du refresh token puis le rejeu de la requête, donc un second envoi du mauvais mot de passe. Un compte né d'un OAuth n'a pas de mot de passe : `hasPassword` (dans `PublicUser`) dit au front s'il doit le demander. **Ce booléen ne décrit que le titulaire de la session** — si `PublicUser` sert un jour à décrire les autres joueurs du lobby, il lui faudra une projection distincte.
  - **L'effacement nomme `content_draws` explicitement** : cette table n'a pas de clé étrangère vers `users` (son `subject` couvre aussi les appareils anonymes), la cascade ne l'emporte donc pas et l'historique de tirage survivrait au compte. Le tout dans une `$transaction`.
  ...
  - Tous les modèles portent un `@@map` snake_case (`users`, `user_avatars`, `refresh_tokens`, `word_pairs`, `content_draws`, `rooms`, `room_players`, `room_votes`). Il n'y a plus de tables `entitlements`, `credit_wallets`, `daily_usage` ni `challenges` — supprimées avec le système de packs/crédits.
...
- **`rooms` module** (`api/src/rooms/`) — salons de jeu en ligne, Socket.IO (`@nestjs/websockets`/`@nestjs/platform-socket.io`, namespace `/rooms`). Création (`POST /rooms`) et jointure (`POST /rooms/:code/join`) restent des routes HTTP — `resolveSubject` a besoin d'une vraie `Response` pour poser le cookie `device_id` d'un hôte ou d'un invité anonyme, ce qu'un handshake de socket ne permet pas. Tout le reste de la partie (config, lancement, révélation, tours, votes, élimination, victoire, replay) passe par `RoomsGateway`, seul transport autoritaire une fois dans la salle. Points à ne pas défaire :
  - **Le serveur fait autorité, jamais le client.** Contrairement au moteur local (`useGame.ts`, côté navigateur), `api/src/rooms/engine/` porte une version serveur des mêmes règles (distribution, ordre de parole, victoire) — portée depuis `useGame.ts`, algorithme identique, testée en miroir (`engine/*.spec.ts`). `client.data.playerId`, fixé une fois pour toutes à la connexion, est la SEULE source de « qui agit » ; jamais un `playerId` pris dans un payload entrant.
  - **`redaction.ts` est le seul chemin vers un `room:state`.** Un agent voit son propre rôle/mot, ceux d'un agent déjà éliminé, et tout le monde une fois en phase victoire — rien d'autre. Aucune diffusion room-wide ne contient jamais `role`/`word` : `RoomsGateway.broadcastState` unicast à chaque joueur connecté sa propre vue rédigée, vers un salon Socket.IO personnel (`player:<id>`) pour que plusieurs onglets d'un même agent reçoivent chacun la leur. Le vote, lui, est public par choix produit — chaque clic est visible en direct, identité comprise (`RoomVote` n'a donc besoin d'aucune rédaction).
  - **`RoomPlayer.playerTokenHash` ≠ cookie `device_id`.** Le premier identifie un SIÈGE dans UNE salle (vit en `localStorage` côté front, transite dans `handshake.auth.token`, haché comme `RefreshToken.tokenHash`) ; le second identifie un APPAREIL pour la déduplication des tirages de mots (`content_draws`). Les confondre casserait soit la reconnexion à la salle, soit l'historique de tirage.
  - **L'hôte tire les mots.** Son `Subject` (`user:<id>` ou `device:<uuid>`) est résolu une seule fois, à `POST /rooms` — le seul moment où un `Response` HTTP existe pour poser `device_id` si l'hôte est anonyme — et stocké dans `RoomPlayer.subjectKey`. `room:start`/`game:replay` reconstruisent ce `Subject` en mémoire et appellent `WordsService.draw()` **en process**, d'où l'export de `WordsService` ajouté à `WordsModule` — ne pas le retirer.
  - **Thème, spicy et difficulté sont configurables en ligne** : `POST /rooms` et `room:configure` acceptent `theme?`, `spicy?`, `difficulty?` — la salle stocke ces valeurs dans `Room.theme`, `Room.spicy`, `Room.difficulty`.
  - **Égalité au vote : jusqu'à 3 tentatives, puis tirage au sort** (`engine/voting.ts`, `MAX_VOTE_ATTEMPTS`). Sans ce plafond, un groupe qui reste bloqué ferait tourner la partie indéfiniment — un cas qui n'existe pas en pass-and-play local (consensus verbal, jamais d'égalité à trancher).
  - **Chaque mutation de jeu passe par un compare-and-swap Prisma**, même idiome que `auth` : `updateMany({ where: { phase: 'vote', ... } })` avant de rebasculer la phase, pour que deux votes qui se croisent ne fassent pas avancer la partie deux fois.
  - Pas de nettoyage automatique des salles abandonnées pour l'instant (aucun TTL) — prévu en fast-follow via `@nestjs/schedule`, dépendance déjà installée mais jusqu'ici inutilisée.
