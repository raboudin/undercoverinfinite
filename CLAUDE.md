...
- `frontend/` — Nuxt 4 app with a ported design system (Tailwind theme tokens + Vue components), **a playable local pass-and-play game** (v1), and a networked online lobby (`/salle`, mode Classique only so far — see below).
- `api/` — NestJS backend with Prisma ORM. Four domain modules: `entitlements` (packs, modes, thèmes, crédits), `words` (le tirage des mots par LLM), `auth` (comptes, profil, JWT en cookies, OAuth Google/Facebook) et `rooms` (salons en ligne, Socket.IO) — see below.

Game state stays client-side **for local pass-and-play only** — the online mode is the opposite, the server is authoritative there (see `rooms` below). Either way, the frontend calls `api/` for **what a player is allowed to do** (`GET /entitlements`) and for **the words of each game** (`POST /words/draw`, which debits a credit — in the online mode it's the host's credit). There is no local word list and no daily batch: words are generated on demand.
...
- **`LobbyScreen` ne propose aucun sélecteur de mode**, contrairement à `SetupScreen` en local : le serveur refuse de toute façon tout mode autre que Classique pour l'instant (voir `ONLINE_ENABLED_MODES` côté API) — proposer le choix laisserait croire à un réglage sans prise.
...
- **`auth` module** (`api/src/auth/`) — comptes, profil et sessions. Login classique (`POST /auth/register|login|refresh|logout`, `GET /auth/me`) plus OAuth Google/Facebook (`GET /auth/{google,facebook}[/callback]`), les deux chemins terminant sur la même émission de tokens. `GET /auth/providers` (public) dit au front quels boutons sociaux ce serveur peut honorer. Le dossier se modifie via `PATCH /auth/me` (nom de code), `PUT|DELETE /auth/me/avatar` (photo) et `DELETE /auth/me` (effacement RGPD) ; `GET /auth/avatar/:userId` sert l'image. Points à ne pas défaire :
  ...
  - **L'effacement nomme `daily_usage` et `content_draws` explicitement** : ces deux tables n'ont pas de clé étrangère vers `users` (leur `subject` couvre aussi les appareils anonymes), la cascade ne les emporte donc pas et la consommation comme l'historique de tirage survivraient au compte. Le tout dans une `$transaction`.
...
- **`rooms` module** (`api/src/rooms/`) — salons de jeu en ligne, Socket.IO (`@nestjs/websockets`/`@nestjs/platform-socket.io`, namespace `/rooms`). Création (`POST /rooms`) et jointure (`POST /rooms/:code/join`) restent des routes HTTP — `resolveSubject` a besoin d'une vraie `Response` pour poser le cookie `device_id` d'un hôte ou d'un invité anonyme, ce qu'un handshake de socket ne permet pas. Tout le reste de la partie (config, lancement, révélation, tours, votes, élimination, victoire, replay) passe par `RoomsGateway`, seul transport autoritaire une fois dans la salle. Points à ne pas défaire :
  ...
  - **`RoomPlayer.playerTokenHash` ≠ cookie `device_id`.** Le premier identifie un SIÈGE dans UNE salle (vit en `localStorage` côté front, transite dans `handshake.auth.token`, haché comme `RefreshToken.tokenHash`) ; le second identifie un APPAREIL pour le quota de crédits. Les confondre casserait soit la reconnexion à la salle, soit le quota anonyme.
  - **L'hôte paie.** Son `Subject` (`user:<id>` ou `device:<uuid>`) est résolu une seule fois, à `POST /rooms` — le seul moment où un `Response` HTTP existe pour poser `device_id` si l'hôte est anonyme — et stocké dans `RoomPlayer.subjectKey`. `room:start`/`game:replay` reconstruisent ce `Subject` en mémoire et appellent `WordsService.draw()` **en process**, d'où l'export de `WordsService` ajouté à `WordsModule` — ne pas le retirer. Le débit-avant/remboursement-sur-échec est déjà géré à l'intérieur de `draw()`, le gateway n'a rien à dupliquer.
  - **Seul le mode `classique` est jouable en ligne pour l'instant** (`ONLINE_ENABLED_MODES` dans `rooms.service.ts`). Chrono (minuteur serveur), défi et pari (mise simultanée) ont déjà leur schéma (`Room.challenge`, `Room.timerSeconds`) mais pas d'événements branchés — les ouvrir demande de lever cette restriction ET d'écrire la logique manquante, pas l'un sans l'autre.
  ...
  - **Chaque mutation de jeu passe par un compare-and-swap Prisma**, même idiome que `entitlements`/`auth` : `updateMany({ where: { phase: 'vote', ... } })` avant de rebasculer la phase, pour que deux votes qui se croisent ne fassent pas avancer la partie deux fois.
