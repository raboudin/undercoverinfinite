...
**Le crédit est l'unité de facturation : 1 crédit = 1 partie**, quel que soit le nombre de mots qu'elle demande. 5 par jour sans pack (avec ou sans compte), 50 par jour avec un pack illimité, plus un solde acheté qui ne se réinitialise pas. Une partie DIY (mots écrits par le joueur) ne consomme rien : elle n'appelle pas l'API.
...
All game state is client-side; the API supplies the player's rights (`useEntitlements`) and the words of each game (`useWords`). No rights reachable and no credits left both block launching a game — there is no offline word fallback by design, except the DIY path where the player types the words.
...
- `app/composables/useGame.ts` — the whole rules engine as a `createGame({ rng })` **factory**, not a module singleton, so every test starts clean and can inject a deterministic PRNG. Phases: `setup → reveal → describe → (bets) → vote → elimination → (describe | victory)`. Civils win at zero live undercovers; undercovers win at parity (`aliveUndercovers >= aliveCivils`), which is also why `configure()` caps undercovers at `Math.floor((n - 1) / 2)` — start at parity and the game would be over before the first round.
  ...
  - `configure()` and `replaySameTeam()` take a `GameSetup` (`{ pair, challenge? }`) as a parameter — the engine no longer picks words itself. The credit is debited **server-side by the draw**, so the page must call `words.draw()` first and only `configure()` on success. Replaying with the same team is a new game and costs a new draw (except DIY).
...
- `app/components/game/` — screen components (`SetupScreen`, `ModeSelector`, `ThemeSelector`, `RevealScreen`, `DescribeScreen`, `BetScreen`, `VoteScreen`, `EliminationScreen`, `VictoryScreen`), auto-imported flat like the other dirs. They're presentational: props in, emits out, no game state of their own except `RevealScreen`'s local "dossier open" flag (which watches `player.id` so the word is hidden again before the phone changes hands), `VoteScreen`'s pending accusation, `BetScreen`'s current bettor and `DescribeScreen`'s countdown. They compose the existing design system only — don't add design-system components here.
...
- **DIY est verrouillé côté client seulement**, et c'est assumé : cette partie n'appelle jamais l'API (ni crédit ni mots), il n'y a donc rien à protéger côté serveur. DIY n'apparaît pas dans la grille des modes — c'est une source de mots, combinable avec n'importe quel mode.
...
- **`entitlements` module** (`api/src/entitlements/`) — packs, modes, thèmes et crédits. Routes : `GET /packs` (vitrine publique), `GET /entitlements` (droits du demandeur, **ouvert aux anonymes**), `POST /packs/:pack/unlock` (compte obligatoire). Points à ne pas défaire :
  ...
  - **Le prompt d'un thème ne sort jamais de l'API** : `catalog()` expose `{id, label, generalist}`, pas `THEMES[id].prompt` (c'est testé en e2e).
