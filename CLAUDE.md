...
- **Une carte ne dit jamais le camp.** `TableSeat` n'a même pas de quoi le porter : retourner la carte d'un éliminé montre son mot, rien d'autre, et un undercover grillé donne exactement le même écran qu'un loyal. Les rôles ne sortent qu'à la victoire, dans `VictoryScreen`. C'est une règle de jeu, pas une préférence d'affichage — `EliminationScreen.spec.ts` compare les deux rendus caractère pour caractère.

##### Menu principal et vitrine des thèmes

- `SetupScreen` tient **deux étapes** : le menu (modes + dossier thématique, rien d'autre) puis la table (noms, undercovers, chrono, DIY, crédits). Tout ce qui dépend des joueurs présents attend la table ; le menu ne pose que la question qu'on se pose avant de sortir le téléphone du sac.
